...
Orbiter needs an Apple ID with an Apple password. A **federated or Managed Apple ID** — one whose sign-in goes to an organisation's identity provider — has no Apple-side password, so the sign-in Orbiter performs has nothing to verify and the account is refused with a sentence saying so. That is deliberate and not planned: the browser-based flow Xcode uses for those accounts is undocumented and would break without warning, and Orbiter exists for a tester re-signing with their own Apple ID because a company team is full. Use a personal Apple ID.
