...
Direct versions are exact and both Cargo/npm lockfiles are tracked. The chosen stable versions build locally; Vite 7 is retained because it supports this host's Node version. No signing dependency is adopted. Read-only discovery now uses exact-pinned `idevice` 0.1.65 with only `usbmuxd` and `ring`. The repository manifest says 0.1.66, but that version was not published when queried; the downloaded 0.1.65 API was compiled and exercised.
