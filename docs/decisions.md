...
- **Seven-day expiry makes refresh a core feature, not a later milestone.** Personal Team profiles expire in seven days, so every tester needs a repeatable weekly re-sign. `renewal.rs` is the part that keeps saying so: it records an installed build's expiry and reports where it stands, and re-signing remains a click rather than a schedule. Documented Personal Team limits also cap 10 App IDs per seven days, 3 registered devices, and 3 installed apps per device.
