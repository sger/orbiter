...
| Unrecognised sign-in failures | A federated Managed Apple ID failed with the unclassified fallback, which named no step because the failure matched nothing in the table and the upstream report was then discarded. Unit tests now cover the two parse stages and the two error variants that fell through, a redacted diagnostic that reproduces this crate's own source literals while withholding interpolated and Apple-provided text, and that the diagnostic never outlives the failure it describes. A browser test covers the disclosure appearing only after a failure. The diagnostic was then read from the live attempt and named the cause: Apple answered the initial login without the SRP salt and reported no error, which is the shape of an account with no Apple-side password. The user confirmed the account authenticates through a browser, as it does for Xcode. Supporting that flow was declined deliberately (see architecture.md), so the case is now identified and refused rather than left unexplained | Not run |
| Re-signing plan | Unit tests cover identifier rewriting, deterministic per-team identifiers, Personal Team capability removal with consequences, blockers, the App ID budget, and the Watch choice. Apple confirmed the capability decisions in a live run: it enabled none on either identifier | Signer run against the company IPA; build installed and launched |
...
| Wi-Fi installation | Unit tests cover collapsing a phone's transports into one entry preferring the cable, never merging two phones, the per-connection operation deadlines, the silence watchdog reporting **unknown** rather than failed, and every connection-specific sentence avoiding advice about a cable that is not in use. Browser tests cover a Wi-Fi iPhone being selectable and reaching the review, a dual-connected phone appearing once, the Wi-Fi cost note, and the recovery block offering to check the connection again. **No installation over Wi-Fi has been performed on hardware**: the transfer, a mid-transfer drop, a drop while iOS is installing, and unplugging the cable between review and install are all unverified | Not run |
...
| Seven-day expiry tracking | Unit tests cover day rounding, the expiry boundary, and a damaged legacy file. Integration tests cover that only a successful install counts down, that a failed or cancelled one never does, that an unknown expiry is silence rather than zero, that the copy still working leads while the dead one stays listed, that another team gets no countdown, and that history survives removing the saved file | Not observed across a real seven-day boundary |
| Library storage limits | Integration tests cover icons kept beside the artifacts rather than inside the manifest, removal taking an app's icon with it, a bounded installation history that never drops an unfinished attempt, a manifest from an older Orbiter opening and from a newer one being refused, and an import that cannot be recorded leaving no bytes behind | Not measured against a large real library |
| Unreferenced managed files | Integration tests cover counting them, leaving them alone at startup, removing them only on request, and never touching a file the library did not name | Not seen after a real interrupted copy |
...
UI tests cover the browser-only unavailable state, synthetic inspection rendering, recovery after a failed inspection, cancellation acknowledgement, and the re-signing controls, plus explicit acknowledgement for every account write and for installation. They deliberately mock IPC; they do not prove native file dialogs, drag/drop, channel transport, or a physical install work. Screenshots generated by these tests are ignored local artifacts.
...
| Phase 3 authentication/signing | Account sign-in, team selection, device registration, certificate, provisioning and the signer implemented and run live; the two-factor path is still unexercised | Designated test account entered through the application after local-authentication disclosure; then certificates/profiles, reviewed registration/signing plan |
| Phase 4 end-to-end | Run once: a re-signed build installed and launched on a physical iPhone. Repeat runs, a second tester, and the seven-day renewal are untested | Working transport and signer, designated device/account, company test credentials and capability owners |
...
4. Confirm every account write is refused until acknowledged, and that renewal is still manual; account/team controls follow the account acceptance procedure below. Device discovery should show real data without account access, internet requests, or pairing mutations.
...
Authentication follow-up: 50 Rust tests pass (two native checks remain opt-in). Added stage-label and explicit federation-error redaction regressions. The reported HTTP 503 still needs a fresh user-initiated sign-in to locate its request stage. Microsoft-federated login is not supported. Provisioning and re-signing were subsequently run live; see the signed-build section below.
