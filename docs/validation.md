...
| Unrecognised sign-in failures | A federated Managed Apple ID failed with the unclassified fallback, which named no step because the failure matched nothing in the table and the upstream report was then discarded. Unit tests now cover the two parse stages and the two error variants that fell through, a redacted diagnostic that reproduces this crate's own source literals while withholding interpolated and Apple-provided text, and that the diagnostic never outlives the failure it describes. A browser test covers the disclosure appearing only after a failure. **The federated account itself is still unexplained**: the diagnostic exists to identify it and has not yet been read from a live attempt | Not run |
