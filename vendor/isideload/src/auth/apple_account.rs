...
impl AppleAccount {
            ...
            match self.login_state.clone() {
                ...
                LoginState::NeedsDevice2FA => {
                    if self.trusted_phone_numbers.is_none() {
                        // SMS discovery is optional for trusted-device verification.
                        // Keep it retryable if unavailable, without blocking Apple's device prompt.
                        self.trusted_phone_numbers = self.get_trusted_numbers().await.ok();
                    }
                ...
                LoginState::NeedsExtraStep(s) => {
                    ...
                    if self.get_pet().is_err() {
                        // Typed so the caller can report the unsupported step without echoing
                        // Apple's server-provided text.
                        return Err(report!(SideloadError::UnsupportedStep(s)).into_dynamic());
                    }
    ...
    async fn login_inner(&mut self, password: &str) -> Result<LoginState, Report> {
        ...
        // The proof request is a separate request and takes its own client-provided data: the
        // one-time password and client time in `cpd` must not be replayed from the init request.
        let proof_anisette = self
            .anisette_generator
            .get_anisette_data(self.grandslam_client.clone())
            .await
            .context("Failed to get anisette data for login")?;
        let proof_cpd = proof_anisette.get_client_provided_data();
        ...
        let req2 = plist!(dict {
            ...
            "Request": {
                ...
                "cpd": proof_cpd, // cpd = client provided data
        ...
        // Orbiter: the proof request used to be the only request carrying `Connection: close`,
        // which made it the one header-level difference between the init request Apple accepts and
        // the proof request Apple answered with HTTP 429 on a first attempt. A hop-by-hop header
        // that forces a fresh connection per proof is exactly the shape an edge rate-limiter
        // penalises, and nothing in the GrandSlam exchange requires it, so both requests now go
        // out identically.
        let response2 = self
            .grandslam_client
        ...
        verifier.verify_server(m2).map_err(|e| {
            report!("Negotiation failed, server proof mismatch: {}", e)
                .context("Apple's sign-in response failed verification.")
        })?;
        ...
        let spd_decrypted = Self::decrypt_cbc(&verifier, spd_encrypted)
            ...
            .context("Apple's sign-in response could not be read.")?;
        let spd: plist::Dictionary = plist::from_bytes(&spd_decrypted)
            .context("Failed to parse decrypted SPD plist")
            .context("Apple's sign-in response could not be read.")?;
